...
import type { Account, Category, Transaction, TransactionEntry, Book, Note } from '@/lib/types';
  ...
  try {
    ...
    if (error.code === 'ENOENT') { // File does not exist
        try {
            ...
            console.error(`Error creating empty file ${filePath}:`, writeError);
            return [];
        }
    }
...
const writeData = async <T>(filePath: string, data: T[]): Promise<void> => {
  try {
    ...
    try {
      ...
      console.error(`Error creating backup for ${filePath}:`, backupError);
    }
...
export const addAccount = async (
  ...
  accountData: Omit<Account, 'id' | 'bookId'>
    ...
    if (accountData.openingBalance && accountData.openingBalance > 0) {
        ...
        const newAccountEntry: TransactionEntry = {
            accountId: newAccount.id,
            amount: accountData.openingBalance,
            type: (accountData.openingBalanceType || 'debit') as 'debit' | 'credit',
        };

        const obeAccountEntry: TransactionEntry = {
...
export const updateAccount = async (bookId: string, accountId: string, data: Partial<Omit<Account, 'id' | 'bookId'>>): Promise<Account> => {
  ...
  if (openingBalanceTxId && newBalance && newBalance > 0) {
      ...
      const newEntries: TransactionEntry[] = [
          { accountId: accountId, amount: newBalance, type: newType as 'debit' | 'credit' },
